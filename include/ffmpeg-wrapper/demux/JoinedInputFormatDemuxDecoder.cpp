...
void video::JoinedInputFormatDemuxDecoder::InitializeVideoDecoderPipe()
{
	...
	if (stream)
	{
		...
		_video_decode_pipe = std::shared_ptr<ThreadDecoderPipe>{new ThreadDecoderPipe{_video_stream_infos}};
...
void video::JoinedInputFormatDemuxDecoder::InitializeAudioDecoderPipe()
{
	...
	if (stream)
	{
		...
		_audio_decode_pipe = std::shared_ptr<ThreadDecoderPipe>{new ThreadDecoderPipe{_audio_stream_infos}};
...
void video::JoinedInputFormatDemuxDecoder::PumpDataToConsumers(std::shared_ptr<base::CancellationToken> cancel_pump)
{
	while (true)
	{
		...
		std::shared_ptr<base::Pump<AVPacketWrapper>> packet_pump{new base::Pump<AVPacketWrapper>{_current_input_format}};
...
void video::JoinedInputFormatDemuxDecoder::AddVideoFrameConsumer(std::shared_ptr<base::IConsumer<AVFrameWrapper>> consumer)
{
	_video_frame_consumer_list.Add(consumer);
}

void video::JoinedInputFormatDemuxDecoder::AddAudioFrameConsumer(std::shared_ptr<base::IConsumer<AVFrameWrapper>> consumer)
{
