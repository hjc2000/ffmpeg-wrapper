...
{
	// 如果有视频流，初始化视频解码管道
	AVStreamWrapper stream = _context->_current_input_format->FindBestStream(AVMediaType::AVMEDIA_TYPE_VIDEO);
	if (!stream.IsNull())
	{
		...
		*/
		...
		_context->_video_decode_pipe = shared_ptr<ThreadDecoderPipe> { new ThreadDecoderPipe { video::DecoderPipeFactory::Instance(), _context->_video_stream_infos } };
		_context->_video_decode_pipe->FrameConsumerList().Add(_context->_video_frame_consumer_list);
		_context->_infinite_packet_pipe->PacketConsumerList().Add(_context->_video_decode_pipe);
	}
	else
	{
...
{
	// 如果有音频流，初始化音频解码管道
	AVStreamWrapper stream = _context->_current_input_format->FindBestStream(AVMediaType::AVMEDIA_TYPE_AUDIO);
	if (!stream.IsNull())
	{
		...
		*/
		...
		_context->_audio_decode_pipe = shared_ptr<ThreadDecoderPipe> { new ThreadDecoderPipe {
			video::DecoderPipeFactory::Instance(),
			_context->_audio_stream_infos
		} };

		_context->_audio_decode_pipe->FrameConsumerList().Add(_context->_audio_frame_consumer_list);
		_context->_infinite_packet_pipe->PacketConsumerList().Add(_context->_audio_decode_pipe);
	}
	else
	{
...
{
	if (_context->_current_input_format == nullptr && _get_format_callback != nullptr)
	{
